...
def extract_raw_genes(groups_df, courses_df, trimester):
    """Advanced gene extraction supporting joint lectures and delivery mode."""
    ...
    groups_by_ep_year: dict[tuple[str, int], list[str]] = {}
    for _, g_row in groups_df.iterrows():
        gname = g_row[group_name_col]
        ep = get_ep(gname).upper().strip()
        year = CURRENT_YEAR - admission_year(gname)
        groups_by_ep_year.setdefault((ep, year), []).append(gname)

    weeks_per_trimester = 10

    # Aggregate lecture info
    joint_lecture_slots: dict[tuple[str, int, str], int] = {}
    joint_lecture_groups: dict[tuple[str, int, str], list[str]] = {}

    for (ep, year), g_list in groups_by_ep_year.items():
        curriculum_trimester = year * 3 + trimester - 3
        ep_courses = courses_df[
            (courses_df["EP"] == ep) & (courses_df[trimester_col] == curriculum_trimester)
        ]
        ...
        for _, c_row in ep_courses.iterrows():
            ...
            if lec_pw > 0 and (ep, year, course) not in joint_lecture_slots:
                joint_lecture_slots[(ep, year, course)] = lec_pw
                joint_lecture_groups[(ep, year, course)] = g_list
    ...
    # Build lecture genes after processing all programmes
    for (ep, year, course), slots in joint_lecture_slots.items():
        row = courses_df[
            (courses_df["EP"] == ep)
            & (courses_df[trimester_col] == year * 3 + trimester - 3)
        ...
        groups = joint_lecture_groups[(ep, year, course)]
